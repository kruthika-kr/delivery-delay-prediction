# 📦 E-Commerce Delivery Delay Prediction

This project predicts whether an e-commerce order will be delivered late using engineered logistics, seller, and product features.

It started as a notebook-based analysis project and was later transformed into a complete machine learning application with:
- feature engineering
- LightGBM model training
- SHAP explainability
- FastAPI backend
- Streamlit dashboard

---

## Problem Statement

Predict whether an e-commerce order will be delivered late using order, seller, product, and delivery-related signals.

---

## Project Workflow

1. Loaded and cleaned raw order data
2. Created a binary target variable: `is_late`
3. Engineered time, seller, and product features
4. Trained and evaluated machine learning models
5. Tuned classification thresholds
6. Saved the trained model
7. Built a FastAPI prediction service
8. Built a Streamlit dashboard for live predictions

---

## Model Performance

- LightGBM ROC-AUC: **0.739**
- Recall at threshold 0.50: **0.417**
- Recall at threshold 0.30: **0.789**

---

## Dashboard Preview

### Main Dashboard

![Dashboard Home](assets/dashboard_home.png)

---

### High Risk Prediction Example

![High Risk Prediction](assets/dashboard_high_risk.png)

---

## API Preview

![FastAPI Docs](assets/fastapi_docs.png)

---

## SHAP Feature Importance

![SHAP Feature Importance](assets/shap_feature_importance.png)

---

## How to Run

### 1. Start the FastAPI backend

```bash
uvicorn api.main:app --reload
